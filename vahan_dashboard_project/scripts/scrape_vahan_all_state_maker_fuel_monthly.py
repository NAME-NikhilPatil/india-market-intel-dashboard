#!/usr/bin/env python3
"""Scrape exact all-state monthly Maker x Fuel data from VAHAN."""
...
def reset_to_first_page(harvester: Harvester) -> None:
    """PrimeFaces sometimes preserves paginator state across month changes."""
...
def scrape_current_table(harvester: Harvester, year: str, month_number: int) -> list[dict]:
    ...
    while True:
        for row in harvester.parse_page(year):
            ...
            rows.append(row)
        print(f"all_state_maker_fuel_month/{year}-{month_number:02d}: page {page}, rows {len(rows)}", flush=True)
...
def scrape_year(year: str, output_dir: Path, max_pages: int | None, delay: float, headful: bool) -> dict:
    ...
    try:
        ...
        return {
            ...
            "report": "all_state_maker_fuel_month",
            "y_axis": "Maker",
            "x_axis": "Fuel",
            "year_type": "C",
            "years": [year],
            ...
            rows.append(
                {
                    ...
                    "source": "all_state_maker_fuel_month",
                    "scraped_at": payload["scraped_at"],
                }
            )
    rows.sort(key=lambda row: (row["month"], row["maker"], row["fuel_group"], row["fuel_type"]))
    output.parent.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    json_output = args.output_dir / "all_state_maker_fuel_month.json"
    csv_output = args.output_dir / "all_state_maker_fuel_month_long.csv"
    ...
    for year in args.years:
        ...
        print(f"Wrote running all-state payload: {len(payload['records'])} table rows, {row_count} long rows", flush=True)

    write_json(payload, json_output)
    row_count = write_long_csv(payload, csv_output)
    print(f"Wrote {len(payload['records'])} all-state table rows to {json_output}")
